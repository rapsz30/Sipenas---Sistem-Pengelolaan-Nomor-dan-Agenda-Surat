...
const KelolaSurat = () => {
    ...
    try {
      ...
      const formattedData = data.map((item: any) => {
        ...
        return {
          ...
          nomor_surat: item.nomor_surat_resmi || "-",
          status: statusFront,
          file: item.file || "",
        };
      });
  ...
  const handleApproveClick = async (surat: any) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/nomor-surat/tersedia/${surat.id}`,
      );
      const result = await res.json();

      if (result.success && result.data.length > 0) {
        setAvailableNumbers(result.data);
        setSelectedNumber(result.data[0]);
      } else {
        setAvailableNumbers([]);
        setSelectedNumber("");
      }
  ...
  const submitApprove = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/admin/pengajuan/${selectedSurat.id}/approve`,
        {
          method: "PUT",
  ...
  const submitReject = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/admin/surat/${rejectSurat.id}/reject`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
                ...
                filteredSuratList.map((surat) => (
                  <tr key={surat.id}>
                    ...
                    <td
                      style={{
                        fontWeight:
                          surat.status === "Selesai" ? "600" : "normal",
                        color:
                          surat.status === "Selesai" ? "#10b981" : "inherit",
                      }}
                    >
                      {surat.status === "Selesai" ? surat.nomor_surat : "-"}
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="action-buttons">
                        <button
                          ...
                          style={{
                            opacity: surat.status !== "Diproses" ? 0.3 : 1,
                            cursor:
          ...
          <div className="modal-content">
            ...
            <div className="modal-actions">
              ...
              <button
                ...
                style={{
                  opacity:
                    isLoadingNumbers || availableNumbers.length === 0 ? 0.6 : 1,
                }}
              >
